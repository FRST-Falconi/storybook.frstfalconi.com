...
export const WrapperTag = styled.div<StatusProps>`
  ...
  ${({ status }) => {
    ...
    else if (status == 'in_progress') return `background-color: #FAE28D;`
...
export const CicleTag = styled.div<StatusProps>`
  ...
  ${({ status }) => {
    if (status == 'completed') return `background-color: #1BA853; `
    else if (status == 'not_completed') return `background-color: #A50000;`
    else if (status == 'in_progress') return `background-color: #FDAE15;`
