      ...
      <S.selectItem id="select-items">
        ...
        <div style={{ display: 'flex', gap: '6px' }}>
          <p>
            {' '}
            {item.name}
            <span style={{ color: '#757575' }}> {item?.description && `(${item.description})`} </span>
          </p>
        </div>
      </S.selectItem>
    )
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)}>
        <S.modalContainer id="container-modal">
          <S.modalHeader id="header-people">
            <p>
              {' '}
              {props.modalTitle ? props.modalTitle : 'Este grupo é administrado por'} {selectedValues.length}{' '}
              {selectedValues.length > 1 ? props.people : props.person}{' '}
            </p>
          </S.modalHeader>
          <S.modalContent>
              ...
              return (
                <S.modalCards style={{ background: index % 2 === 0 ? '#F2F2F2' : '#FFF' }} key={index}>
                  <div style={{ display: 'flex', gap: '12px' }}>
                    {canShowAvatar && <Avatar src={item.avatar} size="50px" />}
