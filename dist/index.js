...
`;
const ContainerThumbnailResposive = styled__default["default"].div `
  display: flex;
  height: 35vh;
  overflow: hidden;
  max-width: 100%;
...
function handleThumbnails(listThumbnails) {
    return (jsxRuntime.jsx(ScrollContainer, { type: 'horizontal', stepMove: 100, isVisibleControlsButtons: true, sizeArrowButton: 80, marginsArrowButton: 1, horizontalMarginInternScroll: '0', marginTopArrrowButton: '-11rem', className: 'scrollThumbnail', children: listThumbnails.map((item, index) => {
            return (jsxRuntime.jsxs(CardThumbnails, { theme: FRSTTheme, onClick: item.handleFunctionThumbnail, children: [jsxRuntime.jsx(ThumbnailHeaderImage, { img: item.imgThumbnails, className: "imageThumbnails" }), jsxRuntime.jsx("h1", { children: item.titleThumbnail }), jsxRuntime.jsxs(DescriptionThumbnails, { className: "containerInformationThumbnails", theme: FRSTTheme, children: [jsxRuntime.jsx("h2", { children: item.titleThumbnail }), jsxRuntime.jsx("p", { children: item.descpThumbnail })] })] }, index));
        }) }));
}
function handleThumbnailsResposive(sliderThumbanils, listThumbnails, widthSlider) {
    return (jsxRuntime.jsx(ContainerThumbnailResposive, { ref: sliderThumbanils, children: jsxRuntime.jsx(framerMotion.motion.div, { className: "motionThumbnails", drag: "x", dragConstraints: { right: 0, left: -widthSlider }, children: listThumbnails.map((item, index) => {
                return (jsxRuntime.jsxs(CardThumbnails, { theme: FRSTTheme, children: [jsxRuntime.jsx(ThumbnailHeaderImage, { img: item.imgThumbnails, className: "imageThumbnails", onClick: item.handleFunctionThumbnail }), jsxRuntime.jsx("h1", { children: item.titleThumbnail }), jsxRuntime.jsxs(DescriptionThumbnails, { className: "containerInformationThumbnails", theme: FRSTTheme, children: [jsxRuntime.jsx("h2", { children: item.titleThumbnail }), jsxRuntime.jsx("p", { children: item.descpThumbnail })] })] }, index));
            }) }) }));
}
function ParticipantThumbnails({ listThumbnails }) {
    const sliderThumbanils = React.useRef();
